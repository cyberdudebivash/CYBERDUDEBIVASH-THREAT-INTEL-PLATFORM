/**
 * SAFE ROLLBACK TARGET (watchdog-v2-safe-rollback) -- DO NOT DEPLOY AS A FEATURE.
 * This is main @ 6ac0385 with every outbound Watchdog webhook path removed:
 * the events delivery loop is gone, deliverWebhook() never calls fetch, and
 * the destinations route answers 503 for every method (so no unsigned row can
 * be created and no stored signing secret can be listed). Everything else is
 * byte-identical to 6ac0385. See deploy/cyber-watchdog/safe-rollback/README.md.
 *
...
export const WATCHDOG_VERSION = "2.0.0-safe-rollback";
...
// SAFE ROLLBACK: never makes a request. Signature kept for importers.
export async function deliverWebhook(_url, _payload, _fetchImpl, _timeoutMs) {
  return { status: "suppressed_safe_rollback", http_status: null };
}
...
export async function routeWatchdog(req) {
  ...
  if (path === "/api/watchdog/destinations") {
    // SAFE ROLLBACK: webhook destinations are unavailable for every method.
    return { status: 503, body: { error: "webhook_delivery_disabled_safe_rollback", message: "Webhook destinations are unavailable while the platform runs its safe rollback build. Nothing is delivered." } };
  }
  ...
  if (path === "/api/watchdog/watches" || path === "/api/watchdog/matches" || path === "/api/watchdog/events" || path === "/api/watchdog/events/ack") {
    ...
    if (path === "/api/watchdog/matches" || path === "/api/watchdog/events") {
      ...
      const quota = quotaForTier(tier);
      // SAFE ROLLBACK: the v2 webhook delivery loop is removed.
