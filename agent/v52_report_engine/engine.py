"""
CYBERDUDEBIVASH SENTINEL APEX v52/v173 - Premium Report Engine
Generates enterprise threat intelligence reports in HTML and STIX formats.

ACCESS GOVERNANCE v173.0:
  Every report generated by this engine MUST include classification,
  required_tier, and access_policy metadata fields. Reports missing these
  fields are non-compliant and will be rejected by the deployment gate.
  Consumers: access_control_policy.validate_report_metadata(report_meta).
...
# ACCESS GOVERNANCE — import policy governor
try:
    ...
    logger_pre = logging.getLogger("report_engine")
    logger_pre.warning("[ACCESS-GOVERNANCE] access_control_policy not found — using defaults")
...
<body>
    ...
    def _build_json_report(self, config: ReportConfig, title: str, report_id: str,
        ...
        # ACCESS GOVERNANCE v173.0 — classify every report
        # Full reports are PRO+. Executive briefings without IOC/detection = PUBLIC summary.
        # Tactical IOC, campaign, vulnerability = always PRO+.
        is_full = config.report_type in ("tactical_ioc", "campaign", "vulnerability")
        policy_type = "full" if is_full else "full"  # all v52 reports are full/PRO+

        if _POLICY_AVAILABLE:
            policy_meta = get_report_classification(policy_type)
        else:
            policy_meta = {
                "classification": config.classification or "TLP:AMBER",
                "required_tier":  "PRO",
        ...
        report_meta = {
            ...
            "title": title,
            "report_type": config.report_type,
            # ACCESS GOVERNANCE v173.0: mandatory classification fields
            "classification":  policy_meta["classification"],
            "required_tier":   policy_meta["required_tier"],
            "access_policy":   policy_meta["access_policy"],
        ...
        # Validate metadata compliance — log warning if non-compliant
        if _POLICY_AVAILABLE:
            is_valid, missing = validate_report_metadata(report_meta)
            if not is_valid:
                logging.getLogger("report_engine").error(
                    f"[ACCESS-GOVERNANCE] Report metadata non-compliant — missing: {missing}"
                )
