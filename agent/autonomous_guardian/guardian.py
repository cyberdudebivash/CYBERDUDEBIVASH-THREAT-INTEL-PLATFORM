    ...
    for run in failed_runs:
        ...
        for job in jobs:
            ...
            for finding in findings:
                ...
                log(f"  [!]  DETECTED: {finding['name']} [{finding['severity']}]")
    ...
    for finding in all_findings:
        ...
        try:
            ...
            if result.get("applied"):
                ...
                log(f"  [i]  Fix not applied: {result.get('reason', 'unknown')}")
        except Exception as e:
            log(f"  ? Fix crashed: {e}")
            fix_results.append({
                "fix_fn": fix_fn_name,
    ...
    if any(f.get("applied") for f in fix_results):
        ...
        if all_pass:
            ...
            if push_result.get("pushed"):
                ...
                log(f"[!]  Fix validated but push failed: {push_result.get('reason')}")
                report["overall_status"] = "FIXED_NOT_PUSHED"
        else:
            log("[!]  Validation failed - not pushing fix to avoid making things worse")
