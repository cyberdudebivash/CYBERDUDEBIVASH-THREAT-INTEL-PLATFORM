...
<div style="background:#06080d; color:#94a3b8; font-family:'Segoe UI',Arial,sans-serif;
    ...
    <p style="font-size:11px; color:#475569; text-align:center; line-height:1.6;">
        (C) 2026 CyberDudeBivash Pvt. Ltd. - Bhubaneswar, Odisha, India<br>
...
<div style="background:#06080d; color:#94a3b8; font-family:'Segoe UI',Arial,sans-serif;
    ...
    <h1 style="color:#ffffff; font-size:20px;">? Thank You - CyberDudeBivash SENTINEL</h1>
    ...
    <p style="font-size:11px; color:#475569; text-align:center;">
        (C) 2026 CyberDudeBivash Pvt. Ltd. | {PLATFORM_URL}
    </p>
</div>
"""
...
def _send_email(to_email: str, to_name: str, subject: str, html_body: str) -> bool:
    ...
    try:
        ...
        if resp.status_code in (200, 202):
            ...
            logger.warning(f"  [!]  Email failed: {resp.status_code}")
            return False
        ...
        if ok:
            ...
            logger.warning(f"  [!]  Email failed: {resp.status_code} {resp.text[:100]}")
        return ok
...
def process_sales(sales: List[Dict], dry_run: bool = False) -> Dict:
    ...
    for sale in sales:
        ...
        if price < UPSELL_THRESHOLD_CENTS:
            ...
            subject = f"Thank You for Supporting CyberDudeBivash SENTINEL APEX ?"
