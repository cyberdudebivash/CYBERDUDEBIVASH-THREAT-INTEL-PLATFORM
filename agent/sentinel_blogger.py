#!/usr/bin/env python3
"""
sentinel_blogger.py — CyberDudeBivash v7.4.1 (FINAL OPERATIONAL)
Orchestrator: Ingestion -> Triage -> Dispatch -> Alert.
"""
...
class MITREMapper:
    def __init__(self): self.db = {"phishing":{"id":"T1566","tactic":"Initial Access"},"c2":{"id":"T1071","tactic":"C2"}}
    def map_threat(self, text): return [v for k,v in self.db.items() if k in text.lower()]
...
class CDBWhitepaper(FPDF):
    def create_report(self, headline, risk, iocs, mitre, filename):
        ...
        os.makedirs("data/whitepapers", exist_ok=True); self.output(os.path.join("data/whitepapers", filename))

mitre_engine = MITREMapper(); pdf_engine = CDBWhitepaper()

def main():
    logger.info("="*60 + "\nAPEX v7.4.1 — PRODUCTION VERIFIED SUCCESS\n" + "="*60)
    try:
        ...
        full_html = generate_full_post_content(intel_items, extracted_iocs, enriched_metadata, visualizer.generate_heat_map(enriched_metadata), stix_id, mitre_data=mitre_context, risk_score=risk_score)
        
        service = get_blogger_service()
        post = service.posts().insert(blogId=os.environ.get('BLOG_ID', BLOG_ID), body={"title": f"[v7.4.1] {headline}", "content": full_html}).execute()

        if post.get("url"):
            json.dump(list(processed), open(STATE_FILE, "w"))
            if risk_score >= 7.0: pdf_engine.create_report(headline, risk_score, extracted_iocs, mitre_context, f"{stix_id}.pdf")
    ...
    except Exception as e: logger.critical(f"APEX CORE FAILURE: {e}"); raise
