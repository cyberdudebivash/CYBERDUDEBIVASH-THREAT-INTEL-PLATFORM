"""
blog_post_generator.py — CyberDudeBivash v4.8 APEX
Technical Content Engine with Forensic Indicator & Reputation Tables.
"""
...
def generate_headline(intel_items: List[Dict]) -> str:
    ...
    return f"CRITICAL: {primary} (+{len(intel_items)-1} Intelligence Nodes)" if len(intel_items) > 1 else f"ADVISORY: {primary}"

def generate_full_post_content(intel_items: List[Dict], iocs: Optional[Dict] = None, pro_data: Optional[Dict] = None) -> str:
    """Renders final HTML with forensic enrichment, Geo-IP context, and VT Reputation."""
    html = [f"""
    <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1a1f36; line-height: 1.8; max-width: 900px; margin: auto; border: 1px solid #e3e8ee; border-radius: 8px;">
        <div style="background: #1a1f36; color: #ffffff; padding: 25px; border-radius: 8px 8px 0 0;">
            ...
            <p style="margin: 5px 0 0; opacity: 0.8; font-family: monospace;">{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC | REPUTATION v4.8</p>
        </div>
    ...
    # 2. Enhanced Forensic & Reputation Table
    if iocs and any(iocs.values()):
        html.append("""
        <h2 style="color: #cf1124; margin-top: 30px; border-left: 5px solid #cf1124; padding-left: 15px;">Actionable Intelligence (IoCs)</h2>
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 12px; background: #ffffff;">
            <thead>
                <tr style="background: #f7fafc; border-bottom: 2px solid #e3e8ee; text-align: left;">
                    ...
                    <th style="padding: 12px;">Provider / ISP</th>
                </tr>
            </thead>
            <tbody>""")
        ...
        for ioc_type, values in iocs.items():
            for val in values:
                # Retrieve contextual data from the pro_data matrix
                meta = pro_data.get(val, {"location": "-", "isp": "-", "reputation": "-"})
                
                # Dynamic coloring for the VT Verdict
    ...
    html.append("""
        <div style="margin-top: 40px; padding: 20px; background: #f7fafc; text-align: center; font-size: 11px; color: #a3acb9;">
            © 2026 CyberDudeBivash Pvt Ltd — Multi-Source Automated Intelligence Engine
        </div>
    </div>""")
    return "".join(html)
