...
DEFAULT_MAX_PUBLIC_MANIFEST_AGE_HOURS = 6.0
...
def stale_manifest_reason(path: Path, now: datetime | None = None) -> str | None:
    ...
    Only a PROVABLY stale file is blocked. Unreadable JSON, a non-object
    document, or a missing/unparseable `generated_at` all return None so
    those files keep exactly their pre-existing upload behaviour."""
    try:
    ...
    if parsed.parse_status != "SUCCESS" or parsed.normalized is None:
        return None
    now = now or datetime.now(timezone.utc)
    age_hours = (now - parsed.normalized).total_seconds() / 3600.0
    limit = max_public_manifest_age_hours()
    if age_hours <= limit:
        return None
    return (
        f"generated_at={raw} is {age_hours:.1f}h old (limit {limit:g}h) -- "
