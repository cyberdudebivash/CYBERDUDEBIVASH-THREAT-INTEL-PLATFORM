...
def extract_from_bundle(bundle_path: Path) -> dict:
    ...
    return {
        ...
        "blog_url": "",  # Can't recover blog URL from STIX â€” will be blank
        "source_url": "",
        "timestamp": timestamp,
...
def main():
    ...
    print("=" * 60)
    print("SENTINEL APEX v131.0 â€” MANIFEST REBUILD FROM STIX BUNDLES")
    ...
    newest_ts = merged[0].get("timestamp", "?")[:19] if merged else "â€”"
    oldest_ts = merged[-1].get("timestamp", "?")[:19] if merged else "â€”"
    ...
    if apply_mode:
        ...
        print(f"[WRITTEN] {MANIFEST_PATH} â€” {len(merged)} entries")
