...
class CopilotEngine:
    ...
    def detection_write_template(self, threat: Dict, question: str = "") -> Dict:
        ...
        _mitre_tags   = "\n  - attack.".join(t.lower() for t in mitre[:5]) if mitre else "discovery"
        _sigma_level  = "critical" if threat.get("risk_score", 0) >= 9 else "high" if threat.get("risk_score", 0) >= 7 else "medium"
