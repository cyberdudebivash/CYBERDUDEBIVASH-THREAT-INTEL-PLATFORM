...
| Ransomware | `/api/v1/intel/ransomware` | `MONITOR ● OPERATIONAL` is separate from activity. Classifier `ransomware-classifier/1.0`: threat_type, tags, malware family, actor, then title; never description. A group is active only when a current ransomware-classified item names it. Victims are not measured (`null`, shown `N/A`) |
...
| Campaigns | `/api/v1/intel/campaigns` | `campaign-evidence/1.0`: campaign id/type/tag, "campaign" or "<actor> operation" in the title, or a named MITRE group. Severity, risk and KEV alone never make a campaign |
