...
| Ransomware | `/api/v1/intel/ransomware` | `MONITOR ● OPERATIONAL` is separate from activity. Classifier `ransomware-classifier/1.0`: threat_type, tags, malware family, actor (unattributed placeholders ignored), then title; never description. A group is active only when a current ransomware-classified item names it. Victims are not measured (`null`, shown `N/A`) |
...
| ATT&CK coverage | `/api/v1/intel/campaigns` `attack_tactics` | `attack-tactics/1.0`, the 14 Enterprise tactics. Per item: `mitre_tactics[].tactic`, else `attck_techniques[].tactic`, else technique ids mapped through the technique/tactic pairs in the same feed generation, else legacy `kill_chain_phases`. Severity never produces a tactic |
| Campaigns | `/api/v1/intel/campaigns` | `campaign-evidence/1.1`: campaign id/type/tag, or "campaign" / "<actor> operation" in the title. A named MITRE group is supporting evidence only. Severity, risk, KEV or an actor label alone never make a campaign |
