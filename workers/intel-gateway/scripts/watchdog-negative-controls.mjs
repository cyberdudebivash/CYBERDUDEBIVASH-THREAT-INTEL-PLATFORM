...
const CONTROLS = [
  ...
  },
  {
    ...
    find: "if (auth.aud === WATCHDOG_SESSION_POLICY.audience && !path.startsWith(\"/api/watchdog\") && !aiFeedSessionRead) {",
    replace: "if (false) {",
    tests: [T("watchdog-e2e.test.js"), T("ai-threat-feed.test.js")],
  },
  {
  ...
  },
  {
    ...
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "    ledger: { ...rest, destinations: all.filter((d) => !isSignedV3(d)) },",
    replace: "    ledger: { ...rest, destinations: all },",
  ...
  },
  {
    ...
    file: "workers/intel-gateway/src/index.js",
    find: "    const watchdogAuth = (requestSelectsTenant(path, request.headers, url.searchParams, auth) || (path === \"/api/watchdog/session\" && method === \"POST\"))\n      ? await resolveMsspMembership(env, auth) : auth;",
  ...
  },
  {
    ...
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "    let currentContext = null;\n    if (pub.serve_live) {",
    replace: "    let currentContext = null;\n    if (true) {",
    tests: [T("watchdog-command-center.test.js")],
  },
  {
  ...
  {
    id: "relevance_rewrites_upstream_severity",
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "    severity: clean(String(item.severity || \"\"), 24) || null,\n    source: clean(String(item.source || \"\"), 80) || null,\n    title: clean(String(item.title || \"\"), 240),\n    lens: hit.lenses,",
    replace: "    severity: relevance.level === \"MENTIONED\" ? \"CRITICAL\" : clean(String(item.severity || \"\"), 24) || null,\n    source: clean(String(item.source || \"\"), 80) || null,\n    title: clean(String(item.title || \"\"), 240),\n    lens: hit.lenses,",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "relevance_label_claims_vulnerable",
    file: "workers/intel-gateway/src/watchdog-relevance.js",
    find: "  MATCHED: \"Matches your exposure profile\",",
    replace: "  MATCHED: \"You are vulnerable\",",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "tenant_profile_isolation_removed",
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "  req.ledgerKey = ledgerKeyFor(req.auth.sub, t.tenant);",
    replace: "  req.ledgerKey = ledgerKeyFor(req.auth.sub, path === \"/api/watchdog/profile\" ? null : t.tenant);",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "history_recomputed_with_current_profile",
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "    events: (state.events || []).map(publicEvent),",
    replace: "    events: (state.events || []).map((e) => ({ ...publicEvent(e), relevance: { ...computeRelevance(e.evidence && e.evidence.item ? e.evidence.item : {}, state.profile), label: \"\" } })),",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "priority_version_dropped_from_snapshot",
    file: "workers/intel-gateway/src/watchdog-relevance.js",
    find: "      priority: s(priorityVersion, 32),",
    replace: "      priority: null,",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "snapshot_bounds_removed",
    file: "workers/intel-gateway/src/watchdog-relevance.js",
    find: "      hits: (watchHits || []).slice(0, 8).map(",
    replace: "      hits: (watchHits || []).slice(0, 1000).map(",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "definition_and_becomes_or",
    file: "workers/intel-gateway/src/watchdog-definition.js",
    find: "  const matched = def.logic === \"OR\" ? checks.some(Boolean) : checks.every(Boolean);",
    replace: "  const matched = checks.some(Boolean);",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "unsupported_field_silently_accepted",
    file: "workers/intel-gateway/src/watchdog-definition.js",
    find: "    if (FORBIDDEN_KEYS.has(k) || !TOP_LEVEL_KEYS.has(k)) return fail(\"unsupported_field\"",
    replace: "    if (false) return fail(\"unsupported_field\"",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "product_match_broadened_to_substring",
    file: "workers/intel-gateway/src/watchdog-definition.js",
    find: "    const v = c.products.find((x) => containsTokens(kp, tokens(x)));",
    replace: "    const v = c.products.find((x) => normText(kevProduct).includes(normText(x)));",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "preview_creates_durable_event",
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "  const ratio = scanned.length ? matched / scanned.length : 0;",
    replace: "  await ledger(req, { type: \"append_events\", events: evaluateWatches([watch], scanned.map(projectItem), pub.feed_generated_at, req.now, \"request\", profile) });\n  const ratio = scanned.length ? matched / scanned.length : 0;",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "preview_registers_scheduler_subject",
    file: "workers/intel-gateway/src/cyber-watchdog.js",
    find: "  await metricsHook(req, { product_previews: 1 });",
    replace: "  await metricsHook(req, { product_previews: 1 });\n  await registerHook(req, { enabled_watches: 1 });",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "r2_list_introduced_on_preview",
    file: "workers/intel-gateway/src/index.js",
    find: "    const feed = needsFeed ? await r2Get(env, LATEST_JSON_KEY) : null;",
    replace: "    if (path === \"/api/watchdog/watches/preview\") await env.INTEL_R2.list({ prefix: \"api/v1/intel/\" });\n    const feed = needsFeed ? await r2Get(env, LATEST_JSON_KEY) : null;",
    tests: [T("watchdog-exposure-relevance.test.js")],
  },
  {
    id: "pro_watch_quota_changed",
    file: "workers/intel-gateway/src/watchdog-policy.js",
    find: "    watches: 25, brief_items: 50,",
  ...
  // ---- SENTINEL APEX UI system (watchdog-apex-ui.test.js) ----
  {
    id: "ui_fresh_led_semantics_removed",
    file: "css/apex-design-tokens.css",
    find: ".ax-led[data-state=\"ok\"], .ax-led[data-state=\"fresh\"], .ax-led[data-state=\"active\"]",
    replace: ".ax-led[data-state=\"ok\"], .ax-led[data-state=\"active\"]",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_unknown_state_rendered_healthy",
    file: "js/apex-status-model.js",
    find: "return { state: 'unknown', label: 'Intelligence status unknown' };",
    replace: "return { state: 'fresh', label: 'Intelligence fresh' };",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_hardcoded_plan_price",
    file: "cyber-watchdog.html",
    find: "<div id=\"plancurrent\"></div>",
    replace: "<div id=\"plancurrent\"></div><p>Enterprise from $499/mo</p>",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_paid_upgrade_bar_rendered",
    file: "index.html",
    find: "if (tier === 'PRO' || tier === 'ENTERPRISE' || tier === 'MSSP') {",
    replace: "if (tier === 'MSSP') {",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_html_escaping_removed",
    file: "cyber-watchdog.html",
    find: "el.appendChild(kid instanceof Node ? kid : document.createTextNode(String(kid)));",
    replace: "if (kid instanceof Node) el.appendChild(kid); else el.insertAdjacentHTML('beforeend', String(kid));",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_mobile_overflow_containment_removed",
    file: "css/apex-design-tokens.css",
    find: ".ax-table-wrap { overflow: auto; max-width: 100%;",
    replace: ".ax-table-wrap {",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_keyboard_focus_style_removed",
    file: "css/apex-design-tokens.css",
    find: ".ax-app :focus-visible, .ax-focusable:focus-visible { outline: none; box-shadow: var(--ax-focus);",
    replace: ".ax-app :focus-visible, .ax-focusable:focus-visible { outline: none;",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_soc2_certified_claim_added",
    file: "cyber-watchdog.html",
    find: "ISO 27001 / SOC 2: aligned, not certified.",
    replace: "ISO 27001 / SOC 2: aligned, not certified. SOC 2 Certified platform.",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ui_fake_metric_added",
    file: "index.html",
    find: "<dd id=\"acs-count\"><span class=\"ax-na\">CHECKING</span></dd>",
    replace: "<dd id=\"acs-count\">1,284</dd>",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
  ...
  {
    id: "ui_card_source_link_scheme_unchecked",
    file: "js/card_renderer.js",
    find: "<a href=\"${esc(sourceHref)}\"",
    replace: "<a href=\"${esc(item.source_url)}\"",
    tests: [T("watchdog-apex-ui.test.js")],
  },
  {
    id: "ai_feed_stale_served_live",
  ...
  {
    id: "ai_feed_free_sees_details",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  if (!f.details) return { ...base, locked: true, checkout: AI_FEED_CHECKOUT.PRO };\n",
    replace: "",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_pro_sees_timeline",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  if (!f.timeline) return { ...paid, timeline_locked: true, checkout: AI_FEED_CHECKOUT.ENTERPRISE };\n",
    replace: "",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_hub_source_url_unchecked",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  if (!source_url) return { ok: false, error: \"source_url_required\" };\n",
    replace: "",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_feed_item_source_unchecked",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  if (!source_url || !id) return null;",
    replace: "  if (!id) return null;",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_ingest_operator_unchecked",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "    if (!req.isOperator) return { status: 403, body: { error: \"operator_required\" } };\n",
    replace: "",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_amber_shown_to_free",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  const visible = f.amber ? merged : merged.filter((i) => i.tlp !== \"AMBER\");",
    replace: "  const visible = merged;",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_item_view_ungated",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "    if (!AI_FEED_FEATURES[tier].item_view) {",
    replace: "    if (false) {",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_cancelled_keeps_tier",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  const tier = effectiveTier(req.auth);",
    replace: "  const tier = AI_FEED_FEATURES[String(req.auth?.tier || \"\").toUpperCase()] ? String(req.auth.tier).toUpperCase() : \"FREE\";",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_price_literal",
    file: "workers/intel-gateway/src/ai-threat-feed.js",
    find: "  const usd = \"$\" + p.usd_monthly;",
    replace: "  const usd = p.usd_monthly === 0 ? \"$0\" : \"$\" + p.usd_monthly;",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_router_operator_bypass",
    file: "workers/intel-gateway/src/index.js",
    find: "      isOperator: path === \"/api/ai-feed/ingest\" && isWatchdogOperator(request, env),",
    replace: "      isOperator: path === \"/api/ai-feed/ingest\",",
    tests: [T("ai-threat-feed.test.js")],
  },
  {
    id: "ai_feed_session_audience_not_extended",
    file: "workers/intel-gateway/src/index.js",
    find: "&& !path.startsWith(\"/api/watchdog\") && !aiFeedSessionRead) {",
