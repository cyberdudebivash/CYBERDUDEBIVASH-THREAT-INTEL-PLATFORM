...
const CONTROLS = [
  ...
  ["a CRITICAL vulnerability counted as a campaign", DC,
    "  const ev = [];\n  if (/campaign/i.test", "  const ev = [];\n  if (item.severity === \"CRITICAL\") ev.push(\"severity\");\n  if (/campaign/i.test"],
  ["an actor label alone counted as a campaign", DC,
    "  if (!ev.length) return ev;",
    "  if (!ev.length) { const g = _namedGroup(item); return g ? [\"mitre_group:\" + g] : ev; }"],
  ["a pipeline campaign id alone counted as a campaign", DC,
    "  if (!ev.length) return ev;",
    "  if (!ev.length) return typeof item.campaign_id === \"string\" && item.campaign_id ? [\"campaign_id\"] : ev;"],
  ["a placeholder label accepted as a named group", DC,
    "  return !s || UNATTRIBUTED_RE.test(s) || /unattr/i.test(s);", "  return !s || UNATTRIBUTED_RE.test(s);"],
  ...
  ["placeholder actor label recorded as ransomware evidence", DC,
    ".filter((v) => !_isPlaceholderLabel(v));\n    for (const v of new Set", ";\n    for (const v of new Set"],
