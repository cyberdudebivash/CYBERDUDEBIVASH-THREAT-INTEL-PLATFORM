...
function generateSyntheticReport(item, reqPath) {
  const title     = (item.title || "SENTINEL APEX Intelligence Report").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const desc      = (item.description || item.apex?.ai_summary || "Threat intelligence report generated by SENTINEL APEX.").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  ...
  const cveArr    = (item.cve || item.cve_ids || []).filter(Boolean).slice(0, 10);
  const ttps      = (item.ttps || item.mitre_tactics || item.ttp_names || []).slice(0, 8);
  const iocCount  = item.ioc_count || 0;
  const tlp       = item.tlp || "TLP:CLEAR";
  const src       = item.source_url || item.report_url || "";
  const srcSafe   = src.startsWith("http") ? src.replace(/"/g, "&quot;") : "";
  const ts        = item.timestamp || item.processed_at || item.published_at || new Date().toISOString();
  const tsDisp    = ts.replace("T", " ").slice(0, 19) + " UTC";
  const itemId    = item.stix_id || item.id || "unknown";
  ...
  const cveHtml   = cveArr.map(c =>
    `<span style="background:rgba(59,130,246,.15);color:#60a5fa;padding:3px 10px;border-radius:4px;font-size:11px;font-weight:700;border:1px solid rgba(59,130,246,.3);font-family:monospace;">${c.replace(/</g,"&lt;")}</span>`
  ).join(" ");

  const ttpHtml   = ttps.map(t =>
    `<div style="padding:8px 12px;background:rgba(139,92,246,.1);border:1px solid rgba(139,92,246,.2);border-radius:4px;font-size:12px;color:#a78bfa;font-family:monospace;">${t.replace(/</g,"&lt;")}</div>`
  ).join("\n");

  const srcLink   = srcSafe
    ? `<a href="${srcSafe}" target="_blank" rel="noopener" style="color:#00d4aa;text-decoration:none;font-weight:700;">View Original Source →</a>`
    : "";
...
<title>${title} — SENTINEL APEX Intel Report</title>
<meta name="robots" content="noindex">
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#0a0d14;color:#c9d4e8;font-family:system-ui,-apple-system,sans-serif;min-height:100vh;line-height:1.6}
  .hdr{background:linear-gradient(135deg,#0d1117 0%,#111827 100%);border-bottom:1px solid rgba(0,212,170,.2);padding:16px 24px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
  .logo{font-family:monospace;font-size:14px;font-weight:900;color:#00d4aa;letter-spacing:2px}
  .badge{font-family:monospace;font-size:10px;padding:4px 12px;border-radius:3px;font-weight:800;letter-spacing:1px}
  .sev-badge{background:${sevColor}22;color:${sevColor};border:1px solid ${sevColor}66}
  .kev-badge{background:rgba(220,38,38,.12);color:#dc2626;border:1px solid rgba(220,38,38,.35);animation:pulse 1.5s infinite}
  @keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(220,38,38,.4)}50%{box-shadow:0 0 10px 3px rgba(220,38,38,.2)}}
  .container{max-width:900px;margin:0 auto;padding:28px 20px}
  .section{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:22px;margin-bottom:20px}
  .section-title{font-family:monospace;font-size:11px;font-weight:800;color:#00d4aa;letter-spacing:2px;text-transform:uppercase;margin-bottom:14px;padding-bottom:8px;border-bottom:1px solid rgba(0,212,170,.15)}
  .report-title{font-size:22px;font-weight:800;color:#f0f4ff;line-height:1.35;margin-bottom:16px}
  .meta-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;margin-bottom:16px}
  .meta-cell{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:6px;padding:12px;text-align:center}
  .meta-label{font-family:monospace;font-size:9px;color:#64748b;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px}
  .meta-value{font-family:monospace;font-size:18px;font-weight:900}
  .risk-bar{height:6px;background:rgba(255,255,255,.07);border-radius:3px;overflow:hidden;margin-top:4px}
  .risk-fill{height:100%;background:${sevColor};border-radius:3px;width:${riskPct}%;transition:width .5s}
  .desc{font-size:14px;color:#a8b8cc;line-height:1.7}
  .tag-row{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}
  .ttp-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:8px}
  .tlp-bar{font-family:monospace;font-size:11px;font-weight:800;padding:8px 16px;border-radius:4px;letter-spacing:1px;${tlp.includes("AMBER")?"background:rgba(245,158,11,.1);color:#f59e0b;border:1px solid rgba(245,158,11,.3)":tlp.includes("RED")?"background:rgba(220,38,38,.1);color:#dc2626;border:1px solid rgba(220,38,38,.3)":"background:rgba(0,212,170,.08);color:#00d4aa;border:1px solid rgba(0,212,170,.2)"}}
  .ftr{text-align:center;padding:24px;font-family:monospace;font-size:10px;color:#374151;border-top:1px solid rgba(255,255,255,.06);margin-top:12px}
  a{color:#00d4aa}
</style>
</head>
<body>
<div class="hdr">
  <div>
    ...
    <div style="font-family:monospace;font-size:10px;color:#4b5563;margin-top:2px;letter-spacing:1px;">THREAT INTELLIGENCE REPORT</div>
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
    <span class="badge sev-badge">${sev}</span>
    ${kev ? '<span class="badge kev-badge">&#9889; CISA KEV</span>' : ""}
    <span class="badge" style="background:rgba(100,116,139,.1);color:#94a3b8;border:1px solid rgba(100,116,139,.2);">RISK ${risk.toFixed(1)}/10</span>
    <span class="tlp-bar">${tlp}</span>
  </div>
</div>
  ...
  <div class="section">
    ...
    <div class="meta-grid">
      <div class="meta-cell">
        ...
        <div class="meta-value" style="color:${sevColor};">${risk.toFixed(1)}</div>
        <div class="risk-bar"><div class="risk-fill"></div></div>
      </div>
      ${cvss > 0 ? `<div class="meta-cell"><div class="meta-label">CVSS</div><div class="meta-value" style="color:${cvss>=9?"#dc2626":cvss>=7?"#ea580c":"#d97706"};">${cvss.toFixed(1)}</div></div>` : ""}
      ${epss > 0 ? `<div class="meta-cell"><div class="meta-label">EPSS</div><div class="meta-value" style="color:${epss>=50?"#dc2626":epss>=10?"#ea580c":"#d97706"};">${epss.toFixed(1)}%</div></div>` : ""}
      <div class="meta-cell">
        <div class="meta-label">Severity</div>
        <div class="meta-value" style="font-size:13px;color:${sevColor};">${sev}</div>
      </div>
      ${iocCount > 0 ? `<div class="meta-cell"><div class="meta-label">IOCs</div><div class="meta-value" style="color:#00d4aa;">${iocCount}</div></div>` : ""}
      <div class="meta-cell">
        <div class="meta-label">Threat Actor</div>
        <div class="meta-value" style="font-size:11px;color:#a78bfa;">${actor.replace(/</g,"&lt;")}</div>
      </div>
      <div class="meta-cell">
        <div class="meta-label">Published</div>
        <div class="meta-value" style="font-size:10px;color:#64748b;">${tsDisp}</div>
      </div>
    </div>
    <div class="desc">${desc}</div>
    ${cveArr.length ? `<div class="tag-row" style="margin-top:14px;">${cveHtml}</div>` : ""}
  </div>

  ${ttps.length ? `
  <div class="section">
    <div class="section-title">&#9673; MITRE ATT&amp;CK Techniques</div>
    <div class="ttp-grid">${ttpHtml}</div>
  </div>` : ""}

  ${kev ? `
  <div class="section" style="border-color:rgba(220,38,38,.3);background:rgba(220,38,38,.04);">
    <div class="section-title" style="color:#dc2626;">&#9889; CISA Known Exploited Vulnerability (KEV)</div>
    <p style="font-size:13px;color:#fca5a5;line-height:1.7;">This vulnerability is listed in the CISA Known Exploited Vulnerabilities Catalog. CISA mandates that all federal agencies remediate this vulnerability. Immediate patching is strongly recommended for all organizations.</p>
  </div>` : ""}

  <div class="section">
    <div class="section-title">&#9675; Advisory Details</div>
    <div style="display:grid;gap:10px;">
      <div style="display:flex;justify-content:space-between;padding:10px;background:rgba(255,255,255,.02);border-radius:4px;border:1px solid rgba(255,255,255,.05);">
        <span style="font-family:monospace;font-size:11px;color:#64748b;">STIX ID</span>
        <span style="font-family:monospace;font-size:11px;color:#94a3b8;">${itemId.replace(/</g,"&lt;")}</span>
      </div>
      <div style="display:flex;justify-content:space-between;padding:10px;background:rgba(255,255,255,.02);border-radius:4px;border:1px solid rgba(255,255,255,.05);">
        <span style="font-family:monospace;font-size:11px;color:#64748b;">Classification</span>
        <span class="tlp-bar" style="padding:2px 10px;font-size:10px;">${tlp}</span>
      </div>
      ${srcSafe ? `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px;background:rgba(255,255,255,.02);border-radius:4px;border:1px solid rgba(255,255,255,.05);"><span style="font-family:monospace;font-size:11px;color:#64748b;">Source</span>${srcLink}</div>` : ""}
      <div style="display:flex;justify-content:space-between;padding:10px;background:rgba(255,255,255,.02);border-radius:4px;border:1px solid rgba(255,255,255,.05);">
        <span style="font-family:monospace;font-size:11px;color:#64748b;">Report Path</span>
        <span style="font-family:monospace;font-size:10px;color:#475569;">${reqPath.replace(/</g,"&lt;")}</span>
      </div>
    </div>
  </div>

  <div class="section" style="border-color:rgba(0,212,170,.15);">
    <div class="section-title">&#9672; Recommended Actions</div>
    <div style="display:grid;gap:8px;">
      ${kev || risk >= 9 ? `<div style="padding:10px 14px;background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.2);border-radius:4px;font-size:13px;color:#fca5a5;">&#9888; <strong>IMMEDIATE ACTION REQUIRED</strong> — Apply patches immediately. Isolate affected systems. Activate incident response procedures.</div>` : ""}
      ${risk >= 7 ? `<div style="padding:10px 14px;background:rgba(234,88,12,.08);border:1px solid rgba(234,88,12,.2);border-radius:4px;font-size:13px;color:#fdba74;">&#9888; <strong>HIGH PRIORITY</strong> — Apply vendor patches within 24–72 hours. Review exposure. Enable enhanced monitoring.</div>` : ""}
      ${risk < 7 ? `<div style="padding:10px 14px;background:rgba(0,212,170,.06);border:1px solid rgba(0,212,170,.15);border-radius:4px;font-size:13px;color:#6ee7b7;">&#9679; <strong>STANDARD REMEDIATION</strong> — Schedule patching in next maintenance window. Review security controls. Monitor for exploitation activity.</div>` : ""}
      <div style="padding:10px 14px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:4px;font-size:12px;color:#64748b;">Apply defense-in-depth: network segmentation &middot; EDR monitoring &middot; threat hunting &middot; IOC blocking &middot; user awareness training.</div>
    </div>
  </div>
...
<div class="ftr">
  CYBERDUDEBIVASH(R) SENTINEL APEX v${PLATFORM_VERSION} &mdash; THREAT INTELLIGENCE PLATFORM<br>
  Report generated: ${new Date().toISOString().replace("T"," ").slice(0,19)} UTC
  &nbsp;&middot;&nbsp; <a href="https://intel.cyberdudebivash.com" style="color:#374151;">intel.cyberdudebivash.com</a>
</div>
</body>
</html>`;
}
  ...
  if (path.startsWith("/reports/")) {
    ...
    if (legacyMatch || (path.endsWith("/") && path.startsWith("/reports/"))) {
      ...
      if (legacyItem) {
        ...
        const r2Key = `reports/2026/06/${fn}`;
