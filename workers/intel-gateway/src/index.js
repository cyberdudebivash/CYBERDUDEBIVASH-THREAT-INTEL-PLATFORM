...
//         uploaded to sentinel-apex-data by r2_upload.py Stage 3.5 / 4a-PDF)
// No auth required -- PDFs are public intel assets (same as HTML reports).
// =============================================================================
async function serveAdvisoryPDF(pathname, env, rid) {
  // Validate the path pattern: /reports/pdf/{id}.pdf
  const pdfMatch = pathname.match(/^\/reports\/pdf\/(intel--[a-f0-9]{16,32}|rpt_[a-f0-9]{8,32})\.pdf$/i);
  if (!pdfMatch) {
    return jsonResponse({
      error:      'invalid_pdf_path',
      message:    'Invalid PDF path. Expected /reports/pdf/{advisory_id}.pdf',
      request_id: rid,
    }, 400);
  }

  const advisoryId = pdfMatch[1];
  const r2Key      = `reports/pdf/${advisoryId}.pdf`;

  // Source 1: R2 INTEL_R2 (sentinel-apex-data) -- primary
  if (env?.INTEL_R2) {
    try {
      const obj = await env.INTEL_R2.get(r2Key);
      if (obj) {
  ...
  // Source 2: GitHub raw gh-pages (fallback -- if PDF landed in dist/)
  try {
    const ghUrl = `https://raw.githubusercontent.com/${CONFIG.GITHUB_REPO}/gh-pages/reports/pdf/${advisoryId}.pdf`;
    ...
    if (ghResp.ok) {
      const pdfBytes = await ghResp.arrayBuffer();
      // Quick sanity: real PDFs start with %PDF-
      const magic = new Uint8Array(pdfBytes.slice(0, 4));
      const isPDF = (magic[0] === 0x25 && magic[1] === 0x50 && magic[2] === 0x44 && magic[3] === 0x46);
      if (isPDF) {
        slog('INFO', 'ADVISORY_PDF', `PDF served from gh-pages: ${advisoryId}`, { rid });
        return new Response(pdfBytes, {
          status: 200,
          headers: {
            'Content-Type':                'application/pdf',
            ...
            'Access-Control-Allow-Origin': '*',
            'X-Gateway':                   `${CONFIG.GATEWAY_NAME}/${CONFIG.GATEWAY_VERSION}`,
            'X-PDF-Source':                'github-pages',
          },
        });
      }
    }
  } catch (e) {
    slog('WARN', 'ADVISORY_PDF', `GitHub raw miss for ${advisoryId}: ${e.message}`, { rid });
  }

  // PDF not yet generated -- return 404 with helpful context
  slog('WARN', 'ADVISORY_PDF', `PDF not found: ${advisoryId}`, { rid });
  return jsonResponse({
    error:      'pdf_not_found',
    message:    `Advisory PDF '${advisoryId}.pdf' is not yet generated. PDFs are produced on each pipeline run. Check back after the next scheduled run, or use the HTML report at /reports/ instead.`,
...
// =============================================================================
// v161.3: serveReportsIndexFile -- public reports index (no auth required)
// Feeds the dashboard REPORTS tab at intel.cyberdudebivash.com.
// Source priority: Cloudflare R2 (primary) -> GitHub raw gh-pages (fallback)
...
async function serveReportsIndexFile(r2Key, env, rid) {
  ...
  try {
    ...
    const ghHeaders = { "User-Agent": `${CONFIG.GATEWAY_NAME}/${CONFIG.GATEWAY_VERSION}` };
    if (env?.GITHUB_TOKEN) ghHeaders["Authorization"] = `token ${env.GITHUB_TOKEN}`;
    const ghResp = await fetch(ghUrl, { headers: ghHeaders, cf: { cacheTtl: 60 } });
    if (ghResp.ok) {
      const body = await ghResp.text();
      return new Response(body, {
        status: 200,
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/api/reports/latest.json" && method === "GET") return withSec(serveReportsIndexFile("api/reports/latest.json", env, rid));
    if (pathname === "/api/reports/index.json"  && method === "GET") return withSec(serveReportsIndexFile("api/reports/index.json",  env, rid));
    if (pathname === "/api/reports/stats.json"  && method === "GET") return withSec(serveReportsIndexFile("api/reports/stats.json",  env, rid));
    ...
    // GET /reports/pdf/{advisory_id}.pdf  -> served from INTEL_R2 (sentinel-apex-data)
    // Generated by scripts/generate_advisory_pdfs.py (Stage 3.2.6) each pipeline run.
    if (pathname.startsWith('/reports/pdf/') && pathname.endsWith('.pdf') && method === 'GET') {
      return withSec(await serveAdvisoryPDF(pathname, env, rid));
    }
