...
const run=document.getElementById('run'),final=document.getElementById('final'),eventLog=document.getElementById('eventLog');
  ...
  function renderExecutiveSummary(result){const r=result||{};setText('execVerdict',String(r.verdict||'AWAITING').toUpperCase(),r.verdict==='malicious'||r.verdict==='suspicious'?'warn':'');setText('execRisk',Number.isFinite(Number(r.max_risk_score))?String(r.max_risk_score):'—',Number(r.max_risk_score)>=7?'warn':'');setText('execMatches',r.match_count!=null?String(r.match_count):'—');setText('execAiMode',r.llm_enhanced===true?'LLM ENHANCED':(r.ai_mode==='deterministic_fallback'?'DETERMINISTIC':'—'),r.llm_enhanced===true?'ok':(r.ai_mode?'warn':''));const negative=r.negative_intelligence&&r.negative_intelligence.assessment;setText('execRecommendation',negative||r.ai_narrative||r.recommendation||'Mission evidence has not been generated yet.')}
...
.executive-summary{display:none;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;padding:12px}.executive-summary>div{border:1px solid rgba(255,255,255,.08);background:rgba(2,10,14,.76);border-radius:10px;padding:10px}.executive-summary span{display:block;font-size:8px;color:var(--muted2);text-transform:uppercase;letter-spacing:.09em}.executive-summary strong{display:block;margin-top:5px;font-size:13px}.executive-summary p{grid-column:1/-1;margin:0;border-left:2px solid var(--mint);padding:10px 12px;background:rgba(66,224,173,.04);font-size:11px;line-height:1.55;color:#d8ebe6}body[data-view=executive] .executive-summary{display:grid}body[data-view=executive] #final{display:none}@media(max-width:700px){.executive-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
<main class="wrap" id="missionShell">
  ...
  <section class="evidence-section">
    ...
    <div class="evidence-grid">
      ...
      <div class="panel"><div class="panel-head"><strong>Final Intelligence / Terminal Evidence</strong><span class="panel-badge" id="finalBadge">AWAITING MISSION</span></div><div class="executive-summary" id="executiveSummary"><div><span>Verdict</span><strong id="execVerdict">AWAITING</strong></div><div><span>Max Risk</span><strong id="execRisk">—</strong></div><div><span>Matches</span><strong id="execMatches">—</strong></div><div><span>AI Mode</span><strong id="execAiMode">—</strong></div><p id="execRecommendation">Mission evidence has not been generated yet.</p></div><pre class="final" id="final">Awaiting a live mission.</pre></div>
    </div>
  </section>
