...
const run=document.getElementById('run'),final=document.getElementById('final'),eventLog=document.getElementById('eventLog');
  ...
  const keyInput=byId('key'),preflightPanel=byId('preflightStatus');
  ...
  function invalidatePreflight(detail){preflight.seq++;if(preflight.controller){preflight.controller.abort();preflight.controller=null}preflight.eligible=false;preflight.key='';run.disabled=true;renderPreflight('unverified','ACCESS NOT VERIFIED',detail||'Complete credential entry to verify SWARM entitlement.','')}
  ...
  async function verifySwarmAccess(key){
    ...
    try{
      ...
      if(r.ok&&body&&body.eligible===true){
        ...
        renderPreflight('verified','SWARM ACCESS VERIFIED',(ent.tier||'PAID')+' · '+quota+expiry,'ok');run.disabled=false;return true
      }
