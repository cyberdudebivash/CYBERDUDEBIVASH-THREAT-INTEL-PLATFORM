...
const run=document.getElementById('run'),final=document.getElementById('final'),eventLog=document.getElementById('eventLog');
  ...
  function setMissionState(value,tone){setText('missionState',value,tone);setText('finalBadge',value,tone);const shell=byId('missionShell');if(shell)shell.dataset.mission=value||'READY';updateOpsTelemetry()}
  function updateClock(){const el=byId('utcClock');if(!el)return;const d=new Date();el.textContent=d.toISOString().slice(11,19)+' UTC'}
  ...
  updateClock();setInterval(updateClock,1000);updateOpsTelemetry();
  hydrateHealth();
...
.fabric-indicator{display:inline-flex;align-items:center;gap:8px}.fabric-led{width:9px;height:9px;border-radius:50%;background:#496067;box-shadow:0 0 0 4px rgba(73,96,103,.08)}.fabric-led[data-state=RUNNING]{background:var(--amber);box-shadow:0 0 18px rgba(255,200,87,.8);animation:ledPulse .9s ease-in-out infinite}.fabric-led[data-state=COMPLETED]{background:var(--mint);box-shadow:0 0 18px rgba(66,224,173,.75)}
...
@media(max-width:1120px){.mesh-core{grid-template-columns:repeat(4,minmax(0,1fr))}.led-strip{grid-template-columns:repeat(2,1fr)}}@media(max-width:700px){.mesh-core,.led-strip{grid-template-columns:repeat(2,minmax(0,1fr))}.topbar{position:relative;top:auto}.hero h1{font-size:42px}}
...
<main class="wrap" id="missionShell">
  ...
  <section class="hero">
    <div>
      ...
      <div class="soc-ribbon" aria-label="Control plane capabilities"><span class="soc-chip"><i></i>SOC 2-ALIGNED EVIDENCE UX</span><span class="soc-chip"><i></i>LIVE CTI</span><span class="soc-chip"><i></i>PRIVATE MESH</span><span class="soc-chip"><i></i>STIX 2.1</span></div>
    </div>
