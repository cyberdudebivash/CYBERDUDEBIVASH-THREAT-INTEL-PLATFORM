      ...
      <div class="sapx-intel-grid">
        ...
        <div class="sapx-intel-cell">
          ...
          <div class="sapx-ic-val">
            <!-- P0 FIX: a bare count number immediately adjacent to a
                 confidence percentage (e.g. "4" next to "97% conf") can be
                 misread as one concatenated number ("497% conf") at a
                 glance. An explicit unit suffix on the count plus a visible
                 separator before the percentage prevents that, and both are
                 only rendered together when there actually are IOCs to
                 report -- avoiding a duplicate "0 IOCs" / "No IOCs" state. -->
            ${item.ioc_count > 0
              ? `<span class="sapx-ioc-num">${esc(String(item.ioc_count))} IOC${item.ioc_count === 1 ? "" : "s"}</span><span class="sapx-ioc-sep">&middot;</span><span class="sapx-ioc-conf">${esc(item.ioc_confidence.toFixed(0))}% conf</span>`
