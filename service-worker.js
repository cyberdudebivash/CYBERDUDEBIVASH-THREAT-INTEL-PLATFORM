// CYBERDUDEBIVASH SENTINEL APEX — Service Worker v201.0
...
const CACHE_VERSION = 'sentinel-apex-v201.0-live';
...
self.addEventListener('install', event => {
  console.log('[SW v201.0] Installing:', CACHE_VERSION);
  self.skipWaiting();
  event.waitUntil(
    caches.open(CACHE_NAME).then(cache =>
      cache.addAll(STATIC_ASSETS).catch(err => {
        console.warn('[SW v201.0] Optional pre-cache failed:', err);
      })
    )
  );
});
...
self.addEventListener('activate', event => {
  console.log('[SW v201.0] Activating:', CACHE_VERSION);
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          ...
          .map(key => {
            console.log('[SW v201.0] Purging stale cache:', key);
...
async function fetchDashboardStats(request) {
  ...
  try {
    ...
    console.warn('[SW v201.0] Stats freshness normalization skipped:', err);
    return response;
  }
}
